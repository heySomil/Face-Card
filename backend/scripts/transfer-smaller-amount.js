...
/**
 * Transfer 0.1 SUI from Wallet B to Wallet A (smaller amount to account for gas)
 */
...
async function transferSUI() {
  try {
    ...
    console.log(`Amount: 0.1 SUI (adjusted for gas fees)\n`);
    ...
    // Convert 0.1 SUI to MIST (1 SUI = 1e9 MIST)
    const amountInMist = Math.floor(0.1 * 1e9);
    ...
    // Split and transfer 0.1 SUI using the first coin
    ...
    if (result.balanceChanges) {
      ...
      result.balanceChanges.forEach(change => {
        ...
        const owner = change.owner === wallets.walletA.address ? 'Wallet A' : 
                     change.owner === wallets.walletB.address ? 'Wallet B' : 
                     change.owner;
